...
- Name: Day4 PW\LoginAction.spec.js >> SauceDemo Authenticated User Tests >> Valid Login
- Location: tests\Day4 PW\LoginAction.spec.js:5:3
...
```ts
  ...
  2  | 
  3  | test.describe("SauceDemo Authenticated User Tests", () => {
  4  |   // Valid Login Test
  5  |   test("Valid Login", async ({ authenticatedPage }) => {
> 6  |     await expect(authenticatedPage).toHaveURL(
     |                                     ^ TypeError: page.context is not a function
  7  |       "https://www.saucedemo.com/inventory.html",
  8  |     );
  9  |     console.log(await authenticatedPage.url());
  10 |   });
  11 | });
  12 |   
  13 | 
